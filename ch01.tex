...
A {\bf function} is a named sequence of statements.
This program defines one function named \java{main}:
...
The name \java{main} is special: when the program runs, it starts at the first statement in \java{main} and ends when it finishes the last statement.
Later, we will see programs that define more than one function.

\index{package}

A {\bf package} is a collection of functions.
This program defines a package named \java{main}.
% LR: 2018-01-09: confirm rules for package names
You can give a package any name you like, but it is conventional to start with a capital letter.
...
In {\tt hello.go}, the outermost braces contain the class definition, and the inner braces contain the method definition.
