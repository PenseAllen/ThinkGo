...
Paste the source code to https://play.golang.org/ and click the *Run* button to
see what happens.
...
practice compiling and running the the program.
...
. Paste the source code of <<lst-forever>> in the `forever.go` file you just created. Save it.
...
. Where would you change <<lst-forever>> to make it show the time every 5
  seconds, instead of every second?
. Change the text used in `now.Format` from `"3:04:05 PM"` to `"15:04:05"` and
  explain what happens with the output.

Date formatting is not the most important topic in Computer Science, so
consider the following exercise completely optional.


===== Exploring `now.Format`

If you want to explore how `now.Format` works in depth, try this format text:
`"01/02 03:04:05PM '06 -0700"`. You can change the order of the elements however
you like, but `1`, `01` will always stand for the month, `2` or `02` represent
the day, `3` or `03` are the hour, etc.
...
spaces as separators between the elements. For example, the date in a formal
letter in the US would be `January 2, 2006`, while in the UK it would look like
`2 January 2006`. In Japan, a common format is `2006年01月02日`, similar to the
Western `2006-01-02` with kanji characters--for year, month, and day--after
each element of the date.
