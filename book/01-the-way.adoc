...
The <<lst-func-main-hex>> below the `main` function from <<lst-hello-go>> in
machine code.

[[lst-func-main-hex,machine code sample]]
.The main function in machine language.
[source]
----
...
the first *assembly language*, a more readable notation low level programming.
...
perform multiplication -- so any multiplication had to be written in machine
code as a sequence of additions,  similar to the long multiplication method we
learn in grade-school.

Another woman, Grace Hopper, changed that when she invented the first
*compiler*: a program that can read a program written in a high level language
and write and equivalent program in machine code. Since then, we are able to
write `x / y` and the compiler then generates all the machine code
instructions needed to divide two numbers in one specific machine.

As you learn to think like a computer scientist, you'll meet some very
powerful idea. This is one:
