...
Besides the `fmt.Prinln` function that we've seen since <<lst-hello-go>>, the
new functions in <<lst-now2>> are `time.Now()` and `time.Sleep(…)`. The `time`
package has several functions related to time keeping and computing durations.
To get the current time, we use `time.Now()`. To make a function pause for a
moment, we use `time.Sleep(…)`, which requires a numeric argument specifying the
duration of the pause. The unit used by `time.Sleep` is the nanosecond, or
10^-9^ (^1^/~1.000.000.000~). For convenience, you can multiply the duration by one of the
special values defined in the `time` package, such as `time.Hour`,
`time.Second`, or `time.Millisecond`footnote[See
