...
need to import two or more packages, they must named one per line in a list
delimited by parenthesis (compare with <<lst-hello-go>>).
...
stored for later use, the playground works on a fixed date and dime--rendering
our <<lst-now>> useless.
...
We will explain all details of <<lst-now2>> in <<repetition-first-take>>.
...
the block "`forever`".footnote:[In practice "`forever`" means until you
interrupt the program with kbd:[Ctrl+C], or some other disruption affects the
program--for example, Windows decides to restart to install the latest updates.]
