...
To avoid that problem, werecommend that you start out running Go through a
...
`*import*`::
  ...
  program. The main function uses the `fmt.Println` function, therefore we need
  to import the `fmt` package, which provides text formatting and output
  functions.

`func`, `package`, and `import` are *keywords*: especial words that define the
structure of a Go program. Go has 25 keywords: less than most popular
programming languages.footnote:[For comparison: Python has 33 keywords, Java has 49 and C++ has 85.]

Programming in Go is writing functions in `.go` files that make up packages.
Usually our functions will use functions from library packages that we import.
In this book we’ll mostly write stand-alone programs, but we’ll use many
...
text that explains the code in human language. Comments have no effect on the
...
----
func main()
{   // WRONG: this { should be in the same line as the func declaration
    fmt.Println("Hello, World!")
}
----
...
_print_ can mean both "`display on the screen`" and "`send to the printer`". In
