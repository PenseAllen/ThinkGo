...
The next section will show how to use the Go compiler so you can build and run
...
keep costs down. For example, the program in listing <<lst-now>> should tell you
the current time, but on the playground it will always claim it is 11:00 PM,
November 10, 2009 -- the date when Go was announced to the public. You can see
the <<lst-now>> program in the playground at https://tgo.li/2rpsSst.
...
Before it can run, Go source code must be translated to machine code by the Go
*compiler*, one of the software tools that is part of the Go installation.

In this book we’ll use two command-line Go tools to compile our programs:
...
least on our machines it displays a different time every time it runs.
In the next exercise will you will build a more interesting variation, the
simplest text-based "`animated`" clock we could make in Go.
...
Listing <<lst-forever>> shows a program that the _Go Playground_ refuses to run.

[[lst-forever]]
.`forever.go`
[source,go]
----
include::{sourcedir}forever/forever.go[]
----

Visit that code in https://tgo.li/2FVjkYZ and click the *Run* button to
see what happens.

However, you can compile and run <<lst-forever>> on your machine using the `go
run` or `go build` commands. That's next.


==== Using `go run`

We will explain all details of <<lst-forever>> in XXX. Now the goal is to
practice compiling and running the program.

To execute <<lst-forever>> with `go run` on your machine, follow these steps:

. Create a folder on your computer named `forever`.
. Use your editor to create a file `forever.go` in the forever folder.
. Paste the source code of <<lst-forever>> in the `forever.go` file you just
  created. Save it.
. Using the command-line, navigate to the `forever` folder.
. Type the command `go run forever.go` to build and execute the program.
...
After completing the previous exercise, you can compile <<lst-forever>> with
`go build` doing this:

. Type the command `go build`. This creates the `forever` executable file
  (`forever.exe` on Windows).
. Run the `forever` or `forever.exe` executable. See examples below.
. To interrupt the program, use the keyboard combination kbd:[Ctrl+C].
...
On the Windows command prompt, `forever.exe` is executed like this--note that
...
. Where would you change the program to show time in a different place in the
  third column? Hint: you need to use a supported time zone name, which consists
  of a region and a city name, for example `"America/Sao_Paulo"`. Wikipedia has
  a https://en.wikipedia.org/wiki/List_of_tz_database_time_zones[list of
  standard time zones].
. What happens if you use a time zone name that is not recognized, like
  `"Middle-earth/Hobbiton"`? Pay close attention to the error message.
. Change the text used in `now.Format` from `"1/2 3:04:05 PM"` to
  `"2006-01-02 15:04:05"` and explain what happens with the output.

Many programming languages have a few mini-languages inside them. Go has the
time formatting mini-language: a formal language based on the idea of expressing
a date/time format through a concrete example, like `"3:04 PM"`.

Time formatting is not the most important topic in Computer Science, so consider
the following exercise optional.


==== Exploring `now.Format`

If you want to explore how `now.Format` works in depth, try this format text:
`"01/02 03:04:05 PM '06 -0700"`. Note the numbers from 1 to 7 in that text. You
can change the order of the elements however you like, but `1`, `01` will always
stand for the month, `2` or `02` represent the day, `3` or `03` are the hour,
etc.

Other special numbers are `2006` for the year with four digits, `15` for 3 PM
in 24-hour format, and `-07` for the time zone offset from UTC.
